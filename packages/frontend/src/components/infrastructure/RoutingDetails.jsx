...
function SecurityGroupBadge({ sg, env, currentProjectId }) {
  ...
  const fetchRules = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetchWithRetry(`/api/${currentProjectId}/infrastructure/${env}/security-group/${sg.id}`)
      if (response.ok) {
...
function ENICard({ eni, env, currentProjectId, showSubnet = false }) {
      ...
      {eni.securityGroups?.length > 0 && (
        <div className="space-y-1">
          ...
          {eni.securityGroups.map((sg) => (
            <SecurityGroupBadge key={sg.id} sg={sg} env={env} currentProjectId={currentProjectId} />
          ))}
        </div>
      )}
 ...
 */
export function SubnetDetails({ subnet, env, currentProjectId }) {
  ...
  const fetchEnis = useCallback(async (ipFilter = '') => {
    ...
    try {
      ...
      const response = await fetchWithRetry(`/api/${currentProjectId}/infrastructure/${env}/enis?${params}`)
  ...
  }, [env, currentProjectId, subnet?.id])
            ...
            {!eniLoading && !eniError && (
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {enis.length === 0 ? (
                  ...
                  enis.map((eni) => (
                    <ENICard key={eni.id} eni={eni} env={env} currentProjectId={currentProjectId} />
                  ))
                )}
              </div>
            )}
 ...
 */
export function VPCDetails({ vpc, env, currentProjectId }) {
  const [enis, setEnis] = useState([])
  const [eniLoading, setEniLoading] = useState(false)
  const [eniError, setEniError] = useState(null)
            ...
            {!eniLoading && !eniError && (
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {enis.length === 0 ? (
                  ...
                  enis.map((eni) => (
                    <ENICard key={eni.id} eni={eni} env={env} currentProjectId={currentProjectId} showSubnet={true} />
                  ))
                )}
              </div>
            )}
