...
export default function App() {
  ...
  const fetchServices = useCallback(async (env) => {
    ...
    try {
      const res = await fetchWithRetry(`/api/services/${env}`)
      const data = await res.json()
  ...
  const fetchPipelines = useCallback(async () => {
    ...
    try {
      const pipelinePromises = projectServices.map(async (service) => {
        const res = await fetchWithRetry(`/api/pipelines/build/${service}`)
  ...
  const fetchImages = useCallback(async () => {
    ...
    try {
      const imagePromises = projectServices.map(async (service) => {
        try {
          const res = await fetchWithRetry(`/api/images/${service}`)
          if (!res.ok) {
  ...
  const fetchInfrastructure = useCallback(async (env, infraConfig) => {
    ...
    try {
      ...
      const url = `/api/infrastructure/${env}${queryString ? `?${queryString}` : ''}`
  ...
  const fetchEvents = useCallback(async (env, hours) => {
    ...
    try {
      ...
      const res = await fetchWithRetry(`/api/events/${env}?hours=${actualHours}${typesParam}${servicesParam}`)
      const data = await res.json()
  ...
  const fetchMetrics = useCallback(async (env, service) => {
    try {
      const res = await fetchWithRetry(`/api/metrics/${env}/${service}`)
      const data = await res.json()
      setMetrics(prev => ({
        ...prev,
  ...
  const fetchDetails = useCallback(async (env, service) => {
    ...
    try {
      const res = await fetchWithRetry(`/api/details/${env}/${service}`)
      const data = await res.json()
